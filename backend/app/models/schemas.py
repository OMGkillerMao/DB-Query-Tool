...
class ExportRequest(BaseModel):
    """Input schema for exporting query results to a file.

    The same SQL that produced the on-screen result is re-executed
    server-side so the exported file is not limited by what the frontend
    currently holds in memory.
    """

    sql: str = Field(..., min_length=1, description="SQL SELECT query to export")
    format: ExportFormat = Field(default=ExportFormat.CSV, description="Export format: csv or json")
    limit: int | None = Field(
        default=None,
        ge=1,
        le=1_000_000,
        description="Maximum rows to export. Defaults to the app-wide query limit. "
        "Only applies when the SQL has no explicit LIMIT clause.",
    )
