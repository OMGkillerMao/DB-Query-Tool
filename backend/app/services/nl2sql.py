...
class NaturalLanguageToSQLService:
    ...
    def __init__(self):
        """Initialize LLM client with support for custom endpoints."""
        self.client = AsyncOpenAI(
            api_key=settings.openai_api_key,
            base_url=settings.llm_base_url,  # None uses OpenAI default
        )
    ...
    def _build_prompt(
        ...
        if db_type == DatabaseType.MYSQL:
            db_name = "MySQL"
            syntax_rules = """3. **ALWAYS wrap identifiers in backticks** — table names, column names, schema names. Example: `db`.`table`.`column` (NOT double quotes, NOT unquoted). MySQL treats double-quoted text as a string literal, which will FAIL.
4. Return valid MySQL syntax (no PostgreSQL `::` casts, no `RETURNING` clause)
5. Use MySQL LIMIT syntax (LIMIT n)
6. Be aware of MySQL-specific features like AUTO_INCREMENT, and string functions like CONCAT()"""
        else:
            db_name = "PostgreSQL"
            syntax_rules = """3. Use double quotes for identifiers if needed
4. Return valid PostgreSQL syntax (you may use `::` casts, `RETURNING`)
5. Use PostgreSQL LIMIT syntax (LIMIT n)"""
    ...
    async def generate_sql(
        ...
        try:
            ...
            response = await self.client.chat.completions.create(
                model=self.model,
                messages=messages,
                temperature=self.temperature,  # Low temperature for consistent SQL generation
