...
export const DatabaseShow: React.FC = () => {
  ...
  const handleExecuteQuery = async () => {
    ...
    try {
      ...
      setExportSql(response.data.sql || sql.trim());
  ...
  const handleExport = async (format: ExportFormat, sqlToExport?: string) => {
    ...
    setExporting(true);
    try {
      const { filename, rowCount } = await exportQuery(id, sqlToUse, format);
      message.success(`Exported ${rowCount.toLocaleString()} rows to ${filename}`);
    } catch (error: any) {
  ...
  return (
    ...
    >
          ...
          <Space direction="vertical" style={{ width: "100%" }} size={24}>
            ...
            {queryResult && (
              <Card
                ...
                extra={
                  <Space>
                    <Button size="middle" onClick={() => handleExport("csv")}>
                      EXPORT CSV
                    </Button>
                    <Button size="middle" onClick={() => handleExport("json")}>
                      EXPORT JSON
                    </Button>
                  </Space>
                }
      ...
      <Modal
        ...
        footer={
          <Space>
            <Button onClick={() => setExportPromptOpen(false)}>LATER</Button>
            <Button
              type="primary"
              loading={exporting}
              onClick={() => {
      ...
      >
        <Text>
          ...
          {queryResult?.rowCount?.toLocaleString() ?? 0} rows as a CSV or JSON
          file?
        </Text>
      </Modal>
    </Show>
