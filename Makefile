.PHONY: help install dev backend frontend test lint format clean db-migrate db-upgrade db-downgrade export
...
# Data export (one-command execute + export)
FORMAT ?= csv
LIMIT ?= 100000
export: ## Execute a SQL query and export the result: make export DB=<name> SQL="<sql>" FORMAT=csv LIMIT=100000
	@if [ -z "$(DB)" ] || [ -z "$(SQL)" ]; then \
		echo "$(YELLOW)Usage: make export DB=<database-name> SQL=\"SELECT * FROM table\" FORMAT=csv$(NC)"; \
		exit 1; \
	fi
	cd $(BACKEND_DIR) && $(UV) run python scripts/export_query.py \
		--db "$(DB)" \
		--sql "$(SQL)" \
		--format $(FORMAT) \
		--limit $(LIMIT)
